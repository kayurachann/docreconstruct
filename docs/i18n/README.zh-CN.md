...
内容，且不会露出公式排版用的对齐标记。生成该示例时项目 QA 的 34 项实测检查全部通过，
检查项现已增至 39 项。存档的 92.58% 早于当前指标，不可直接比较：在
`VISUAL_METRIC_VERSION = 2.2` 下重新测量，同一文件得分为 **31.46%**，因为
2.2 评估前景、边缘与区域的吻合度而非原始像素吻合度（其像素相似度为
94.71%）。三个示例的重测表见
[docs/showcases/README.md](../showcases/README.md)。自动相似度分数不能证明
每个字形或数学陈述在语义上都正确。

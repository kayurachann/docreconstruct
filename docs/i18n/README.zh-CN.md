...
首个独立于精选示例的公开基准结果已经跑完 OmniDocBench 官方演示集的
**全部 18 页**。该测试采用真值辅助重建（`oracle_reconstruction`）：使用
标准 Markdown 和标准几何标注来单独考察重建能力，因此**不是** OCR 对比测试。

| 结果 | 实测值 |
| --- | ---: |
| 流程成功运行 | **8/18（44.44%）** |
| 通过全部已测验收条件 | **1/18（5.56%）** |
| 严格证据对齐失败 | **10/18** |
| LibreOffice 图像指标 v2.1（包含失败样本） | **0.104675** |

失败样本仍保留在报告中，其图像质量按零分计入。完整内容见[测试规程、结果与
失败清单](../../benchmark/omnidocbench-demo/README.md)。这组结果不能用于和
Docling、MinerU 或 Marker 比较；只有在不接触标准答案、完全从输入图像生成
Markdown 和 JSON 后，才具备有效比较的前提。
